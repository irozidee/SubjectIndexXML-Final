...
Public Class frmMain
    ...
    Private Sub btnBrowse_Click(sender As Object, e As EventArgs) Handles btnBrowse.Click
        ...
        If fbd.ShowDialog = Windows.Forms.DialogResult.OK Then
            ...
            For Each strfilename As String In files
                ...
                Dim patternHTML As String = "<.*?.>"
                judgment_name = Regex.Replace(myxmlcase.JUDGMENT_NAME, patternHTML, "").Replace(Environment.NewLine, " ")
                ...
                Try
                    ...
                    Dim strSQL As String = "INSERT INTO xml_cases(datafilename, citation, judgment_name, judgment_language, judge_name, court_type, firstletter) " & _
                        "VALUES ('" & datafilename & "','" & utility.PrintCitation(datafilename) & "','" & StrConv(EscapeString(StrConv(judgment_name, VbStrConv.ProperCase).Replace("  ", " ").Replace("<B>", "").Replace("</B>", "").Replace("&amp;Gt;", "")), VbStrConv.ProperCase) & "','" & judgment_language & "','" & _
                        EscapeString(judge_name.Replace("<NAME>", "").Replace("</NAME>", "").Replace("  ", " ")) & "','" & court_type & "','" & firstLetter & "')"
    ...
    Private Sub Update_API_Data()
        ...
        Using webClient = New System.Net.WebClient()
            ...
            For Each legislation As LegislationListC In legislationList
                ...
                data.Add("('" & legislation.datafilename & "','" & EscapeString(legislation.title).Replace("  ", " ") & "','" & legislation.legislationtype & "')")
                counter += 1
        ...
        Using webClient = New System.Net.WebClient()
            ...
            For Each cases As CasesListC In casesList
                ...
                data.Add("('" & cases.datafilename & "','" & EscapeString(cases.title).Replace("  ", " ") & "','" & cases.citation & "')")
                counter += 1
                If counter Mod 1000 = 0 Then
                    extendSQL = String.Join(",", data.ToArray())
                    'extendSQL = extendSQL.Replace("|", """")
    ...
    End Sub
    Private Function RemoveBracketCitation(ByVal input As String) As String
        Return input.Replace("(refd)", "").Replace("(folld)", "").Replace("(distd)", "").Replace("(not folld)", "").Replace("(Distd)", "").Replace("Folld", "").Replace("Not Folld", "").Replace("Not folld", "").Replace("Ovrld", "").Replace("ovrld", "")
    End Function
        ...
        For Each strfilename As String In files
            ...
            For Each referredCase As String In referredCasesList
                ...
                Try
                    ...
                    If referredCitation.Contains("MLRA") Or referredCitation.Contains("MELR") Or referredCitation.Contains("MLRH") Or referredCitation.Contains("SSLR") Or referredCitation.Contains("MLRS") Then
                        referredTitle = subjectIndexCls.GetJudgmentNameFromCitation(referredCitation)
                    End If

                    If referredTitle = "" Then
                        referredTitle = referredCase.Split({"#"}, StringSplitOptions.RemoveEmptyEntries)(1)
                    End If

                    referredTitle = ReplaceProperWords(StrConv(referredTitle, VbStrConv.ProperCase)).Trim
                    ...
                    insertSQL = "insert into ref_cases (source_citation, source_datafilename, referred_citation, referred_title, referred_type) values('" & utility.PrintCitation(referredDatafilename) & "','" & referredDatafilename & "','" & RemoveBracketCitation(referredCitation) & "','" & EscapeString(referredTitle.Trim()) & "','" & referredType & "')"
    ...
    Private Sub Extract_Ref_Legislation()
        ...
        queryInsert = "insert into ref_legislations(source_citation, source_datafilename, legis_origin, legis_title, legis_sub_no, legis_sub_no_type, legis_link_text) "

        Dim querySelect As String = ""
        querySelect = "select source_citation, source_datafilename, legislink_origin, legislink_title, legislink_exactvalue, legislink_mainchar, legislink_text from tmp_ref_leg "

        subjectIndexCls.Insert(queryInsert & querySelect)
    ...
    Private Sub Extract_Subject_Index()
        ...
        For Each strfilename As String In files
            ...
            Try
                ...
                judgname = judgnameformat(StrConv(judgname, VbStrConv.ProperCase))
                ...
                If patternSubjectIndex = 1 Then ' whenever pattern is headnote//catchword/subjectindex is detected
                    ...
                    For Each node As XmlNode In catchwordNodeList
                        strCatchword = node.InnerText.Replace("Amp;", "").Replace(": -", ":")
                        strsubjindx = strCatchword.Split({" - "}, StringSplitOptions.None)(0)
                        strToRemove = strsubjindx & " - "
                        level1 = node.InnerXml.Replace(":</b> -", ":</b> ").Split({"<SUBJECT_INDEX>", "</SUBJECT_INDEX>"}, StringSplitOptions.None)(1)
                        ...
                        End If
                        If cbTranslateSubjectIndex.Checked = True Then
                            sql = "INSERT INTO subject_index(source_citation, source_datafilename, subject_index, level1, level2, summary) " & _
                            "VALUES ('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & EscapeString(strsubjindx.ToUpper) & "','" & _
                            EscapeString(subjectIndexCls.Translator_Subject_Index(Extractor.StringRemoveTags(level1)).Replace("  ", " ")) & "','" & EscapeString(Extractor.StringRemoveTags(level2).Replace("  ", " ")) & "','" & EscapeString(strsummary.Replace("  ", " ")) & "')"
                        Else
                            sql = "INSERT INTO subject_index(source_citation, source_datafilename, subject_index, level1, level2, summary) " & _
                                "VALUES ('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & EscapeString(strsubjindx.ToUpper) & "','" & _
                                EscapeString(Extractor.StringRemoveTags(level1)).Replace("  ", " ") & "','" & EscapeString(Extractor.StringRemoveTags(level2).Replace("  ", " ")) & "','" & EscapeString(strsummary.Replace("  ", " ")) & "')"
                        End If
                ...
                ElseIf patternSubjectIndex = 2 Then ' subject_index tag with dash
                    ...
                    For Each node As XmlNode In subjectIndexNodeList
                        strCatchword = ""
                        strsummary = ""
                        strsubjindx = node.InnerText.Replace(": -", ":")
                        ...
                        If cbTranslateSubjectIndex.Checked = True Then
                            sql = "INSERT INTO subject_index(source_citation, source_datafilename, subject_index, level1, level2, summary) VALUES ('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & EscapeString(strsubjindx.ToUpper) & "','" & EscapeString(subjectIndexCls.Translator_Subject_Index(Extractor.StringRemoveTags(level1))) & "','" & EscapeString(Extractor.StringRemoveTags(level2)) & "','" & EscapeString(strsummary) & "')"
                        Else
                            sql = "INSERT INTO subject_index(source_citation, source_datafilename, subject_index, level1, level2, summary) VALUES ('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & EscapeString(strsubjindx.ToUpper) & "','" & EscapeString(Extractor.StringRemoveTags(level1)) & "','" & EscapeString(Extractor.StringRemoveTags(level2)) & "','" & EscapeString(strsummary) & "')"
                        End If
    ...
    End Function
    Public Function judgnameformat(strjdjname As String) As String
        strjdjname = StrConv(strjdjname, VbStrConv.ProperCase)
    ...
    Private Function ReplaceProperWords(ByVal strReturn As String) As String
        ...
        .Replace("Bsn ", "BSN").Replace(" V ", " v. ").Replace(" Yb ", " YB ").Replace("()", "").Replace("((", "(").Replace("))", ")").Replace("amp;amp;", "amp;") _
        ...
        For Each linkNode As XmlNode In linkNodeList 'Link
            If linkNode.InnerXml.Contains("<LINK") Or linkNode.InnerXml.Contains("<Link") Then '================CONTAINS LOCAL CASE LINKING======================================
                ...
                For Each casenode As XmlNode In caseNodelist
                    Dim allText As String = casenode.ParentNode.InnerText

                    Dim currentText As String = casenode.InnerText.Replace("Dirujuk", "refd").Replace("dirujuk", "refd")
                    refCaseCitation = Extractor.FindCitation(currentText).Replace("#", "; ")

                    If refCaseCitation.Length > 2 Then
                        currentText = currentText.Replace(refCaseCitation, "")
                    End If

                    refCaseType = RegexC.Cases.Find_Referred_Cases_Type_Regex(allText)
                    If refCaseType.Length > 3 Then
                        currentText = currentText.Replace(refCaseType, "")
                    End If

                    refCaseTitle = currentText.Trim
            ...
            Else '=============FOREIGN CASES===================
                ...
                'refCaseType = RegexC.Cases.Find_Referred_Cases_Type_Regex(refCaseTitleWithCitation)
                'Dim refCaseTitleWithType As String = Extractor.ReferredCases.FindReferredCasesTitle(refCaseTitleWithCitation)

                ' refCaseTitle = refCaseTitleWithType.Replace(refCaseType, "").Replace("<p>", "").Replace("</p>", "")
                Dim currentText As String = linkNode.OuterXml.Replace("Dirujuk", "refd").Replace("dirujuk", "refd")
                refCaseCitation = Extractor.FindCitation(currentText).Replace("#", "; ")
                If refCaseCitation.Length > 2 Then
                    currentText = currentText.Replace(refCaseCitation, "")
                End If

                refCaseType = RegexC.Cases.Find_Referred_Cases_Type_Regex(currentText)
                If refCaseType.Length > 3 Then
                    currentText = currentText.Replace(refCaseType, "")
                End If
    ...
    Private Sub Extract_Temp_Ref_Legislation_Headnote_To_DB(ByVal XMLFilePath As String)
        ...
        For Each node As XmlNode In nodeList
            Dim listLegis As String()
            Dim legis_words As String = node.InnerText
            If node.InnerText.Contains("Legislation referred to:") Or node.InnerText.Contains("Perundangan yang dirujuk:") Then
                ' this ignore because its not contain any legislation
            Else
                ...
                Try
                    If legis_words.Contains(",") Then
                        listLegis = legis_words.Split(",")
                        Dim legisSubNoList As String()
                        Dim tmpLegisTitle As String = ""
                        Dim tmpLegisNoTitle As String = ""

                        If listLegis.Count > 1 Then

                            Dim startIndexSubNo As Integer = 0
                            Dim c As Integer = 0
                            For Each s As String In listLegis
                                If tmpLegisTitle = "" Then
                                    tmpLegisTitle = s
                                Else
                                    If Extractor.ReferredLegislations.Legislation_Keyword_Found(s) = False Then 'still looping to title
                                        tmpLegisTitle = tmpLegisTitle & "," & s & ","

                                    Else ' keyword found title stop here
                                        startIndexSubNo = c + 1
                                        Exit For
                                    End If
                                End If
                                'If Extractor.ReferredLegislations.Legislation_Keyword_Found(s) = False Then
                                '    tmpLegisTitle = tmpLegisTitle & listLegis(Array.IndexOf(listLegis, s))
                                'End If
                                c = c + 1
                            Next
                            If tmpLegisTitle.StartsWith(",") Then
                                tmpLegisTitle = tmpLegisTitle.TrimStart(",")
                            End If

                            If tmpLegisTitle.Trim.EndsWith(",") Then
                                tmpLegisTitle = tmpLegisTitle.TrimEnd(",")
                            End If

                            legis_title = tmpLegisTitle.Trim

                            tmpLegisNoTitle = legis_words.Replace(legis_title & ",", "")
                            legisSubNoList = tmpLegisNoTitle.Split(",")
                            ' MsgBox(tmpLegisNoTitle & " ------ " & legis_title)
                        End If

                        Dim tmpMainSubNo As String = ""
                        Dim tmpMainChar As String = ""
                        Dim tmpSubNo As String = ""
                        Dim tmpSubValue As String = ""
                        For Each SubNo As String In legisSubNoList
                            SubNo = SubNo.Trim

                            '==> ss 102
                            '==> 123
                            '==>(1)|(b)|(iii)
                            If SubNo.ToLower.StartsWith("o ") Then
                                tmpMainSubNo = SubNo
                                tmpSubValue = SubNo
                            Else 'NOT Start with O
                                If Extractor.ReferredLegislations.Legislation_Keyword_Found(SubNo) = True Then
                                    tmpMainChar = Extractor.ReferredLegislations.Extract_Legislation_Sub_No_Type(SubNo)
                                    tmpMainSubNo = SubNo
                                    tmpSubValue = SubNo
                                Else 'Extractor.ReferredLegislations.Legislation_Keyword_Found(SubNo) = false
                                    If IsNumeric(SubNo(0)) = True Then
                                        If tmpMainSubNo.ToLower.StartsWith("o ") Then
                                            ' tmpSubValue = RegexC.Legislation.Parse_Legislation_Order_Value(tmpMainSubNo, SubNo)
                                            If tmpMainSubNo.Contains(" rr ") Or tmpMainSubNo.Contains(" r ") Then
                                                tmpSubValue = RegexC.Legislation.Parse_Legislation_Order_Value(tmpMainSubNo, SubNo)
                                            Else ' its either contains r or empty only
                                                tmpSubValue = RegexC.Legislation.Parse_Legislation_Value(tmpMainSubNo, SubNo)
                                            End If
                                        Else
                                            tmpMainSubNo = SubNo
                                            tmpSubValue = RegexC.Legislation.Parse_Legislation_Value(tmpMainSubNo, SubNo)
                                    ...
                                    Else
                                        If SubNo.StartsWith("(") Then
                                            If tmpMainSubNo.ToLower.StartsWith("o ") Then
                                                ...
                                                End If
                                            Else
                                                If IsNumeric(tmpMainSubNo(0)) = True And tmpMainSubNo.Contains("(") = True Then
                                                    tmpMainSubNo = RegexC.Legislation.Parse_Legislation_Value(tmpMainSubNo, SubNo)
                                                    tmpSubValue = RegexC.Legislation.Parse_Legislation_Value(tmpMainSubNo, SubNo)
                                                ElseIf IsNumeric(tmpMainSubNo(0)) = True And tmpMainSubNo.Contains("(") = False Then
                                                    tmpSubValue = tmpMainSubNo & SubNo
                                                ElseIf IsNumeric(tmpMainSubNo(0)) = False And tmpMainSubNo.Contains("(") = True Then
                                                    tmpSubValue = RegexC.Legislation.Parse_Legislation_Value(tmpMainSubNo, SubNo)
                                                Else
                                                    tmpSubValue = RegexC.Legislation.Parse_Legislation_Value(tmpMainSubNo, SubNo)
                                                End If
                                        ...
                                        End If
                                    End If
                                End If 'Extractor.ReferredLegislations.Legislation_Keyword_Found(SubNo) = True
                            End If 'SubNo.ToLower.StartsWith("o ")

                            If Extractor.ReferredLegislations.Legislation_Keyword_Found(tmpSubValue) Then
                                tmpSubValue = tmpSubValue.Replace("ss ", "s ").Replace("rr ", "r ").Replace("items ", "item").Replace("arts ", "art").Replace("paras ", "para ").Replace("regs ", "reg ")
                            Else
                                tmpSubValue = Extractor.ReferredLegislations.Extract_Legislation_Sub_No_Type(tmpMainChar) & " " & tmpSubValue
                            End If

                            If legis_title = "National Land Code 1965" Then
                                legis_title = "National Land Code"
                            ElseIf legis_title = "Rules of High Court 1980" Then
                                legis_title = "Rules of The High Court 1980"
                            End If

                            insertSQL = "insert into tmp_ref_leg(source_citation, source_datafilename, legislink_filename, legislink_mainno, legislink_mainchar, legislink_subno, legislink_subvalue, legislink_exactvalue, legislink_title, legislink_origin, legislink_text) values('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & legis_filename & "','" & tmpMainSubNo & "','" & tmpMainChar & "','" & SubNo & "','" & tmpSubValue & "','" & tmpSubValue & "','" & EscapeString(subjectIndexCls.Translator_Legislation(legis_title)) & "','" & legis_origin & "','" & EscapeString(legis_words) & "')"
                            subjectIndexCls.Insert(insertSQL)
                        Next
                    End If 'close if legis_text.Contains(",")
                Catch ex As Exception
                    My.Computer.FileSystem.WriteAllText("errorextract.txt", ex.Message.ToString & " " & legis_words & Environment.NewLine, True)
                End Try
            End If
        Next
    ...
    Private Sub btnPrepare_Click(sender As Object, e As EventArgs) Handles btnPrepare.Click
        ...
        If cbPCases.Checked = True Then
            ...
            QuerySelect = "select datafilename, citation, judgment_name, judgment_language, judge_name, court_type, LEFT(referred_title, 1), referred_citation, referred_title, referred_type " & _
        ...
        ' ==> LEGISLATION
        If cbPLegislation.Checked = True Then
            ...
            "select datafilename, citation, judgment_name, judgment_language, judge_name, court_type, LEFT(legis_title, 1), legis_title, legis_filename, legis_sub_no, legis_sub_no_type, legis_link_text " & _
        ...
        ' ==> SUBJECT INDEX
        If cbPSubjectIndex.Checked = True Then
            ...
            "select datafilename, citation, judgment_name, judgment_language, judge_name, court_type, LEFT(subject_index, 1), subject_index, level1, level2, summary " & _
        ...
        InfoTextProgress("All Preparing Data successfully finished")
    End Sub
    ...
    Private Sub Write_XML_Document()
        ...
        Try
            Table_Of_Subject_Index_Level1_List = subjectIndexCls.Table_Of_Content_Subject_Index()
        ...
        Try
            Table_Of_Cases_List = subjectIndexCls.Table_Of_Content_Cases_Title_With_Citation()
            Dim title As String = ""
            Dim cite As String = ""
            Dim court_type As String = ""
            For Each CaseTitle As String In Table_Of_Cases_List
                title = CaseTitle.Split("#")(0)
                cite = CaseTitle.Split("#")(1)
                court_type = CaseTitle.Split("#")(2)
                writer.WriteStartElement("p")
                writer.WriteStartElement("ANORMAL")
                ...
                If cboSelection.Text = "CONSOLIDATED INDEX" Then
                    writer.WriteString(ReplaceProperWords(StrConv(title, VbStrConv.ProperCase)) & Microsoft.VisualBasic.vbTab & court_type & "   " & StrConv(cite, VbStrConv.Uppercase))
                Else
                    writer.WriteString(ReplaceProperWords(StrConv(title, VbStrConv.ProperCase)) & Microsoft.VisualBasic.vbTab & StrConv(PrintingC.PageNo_From_Citation(cite), VbStrConv.Uppercase))
                End If
    ...
    Private Sub WriteXMLCode_Cases_Judicially()
        ...
        For Each firstLetter As String In LetterList
            ...
            For Each Case_Judicially As String In Cases_Judicially_List
                ...
                Dim cCitation As String = subjectIndexCls.CasesJudicial_Citations(cTitle)
                Dim cReferType As String = Case_Judicially.Split({"#"}, StringSplitOptions.None)(2)
                Dim printString As String = cTitle.Replace("amp;", "") & " " & cCitation & " " & cReferType & ""
                writer.WriteStartElement("p")
                writer.WriteStartElement("CBOLD")
                writer.WriteString(printString.Replace("(refd) (refd)", "(refd)"))
                writer.WriteEndElement()
                writer.WriteEndElement()
                writer.WriteString(Environment.NewLine)
    ...
    Private Sub WriteXMLCode_Subject_Index()
        ...
        Subject_Index_Level1_List = subjectIndexCls.SubjectIndex_Level1()
        For Each Level1 As String In Subject_Index_Level1_List
            writer.WriteStartElement("p")
            writer.WriteStartElement("EBOLD")
            writer.WriteString(StrConv(Level1.Replace("Amp;", "").Replace("WORD AND PHRASES", "WORD & PHRASES"), VbStrConv.Uppercase).Replace("AMP;", ""))
            writer.WriteEndElement()
            writer.WriteEndElement()
            writer.WriteString(Environment.NewLine)
            writer.WriteString(Environment.NewLine)

            Subject_Index_Level2_List = subjectIndexCls.SubjectIndex_Level2(Level1)
            For Each Level2 As String In Subject_Index_Level2_List
                ...
                For Each Summary As String In Subject_Index_Summary_List
                    ...
                    For Each Data_Coll In Subject_Index_Data_Coll
                        ...
                        writer.WriteString("(" & judgnameformat(Subject_Index_Judge_Name.Replace("''", "'") & ")"))
                        If cboSelection.Text = "CONSOLIDATED INDEX" Then
    ...
    Private Sub Button1_Click_4(sender As Object, e As EventArgs) Handles Button1.Click
        Extract_Temp_Ref_Legislation_Headnote_To_DB("temp\MLRH_2017_6_376.xml")
    End Sub

    Private Sub EXTRACTREFLEGISToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EXTRACTREFLEGISToolStripMenuItem.Click
        Extract_Ref_Legislation()
    End Sub

    Private Sub EXTRACTREFCASESToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EXTRACTREFCASESToolStripMenuItem.Click
        Extract_Ref_Cases()
    End Sub

    Private Sub EXTRACTSUBJECTINDEXToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EXTRACTSUBJECTINDEXToolStripMenuItem.Click
        Extract_Subject_Index()
    End Sub
    ...
    Private Sub LegislationsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles LegislationsToolStripMenuItem.Click
        Keyword_Legislations.ShowDialog()
    End Sub

    Private Sub SubjectIndexToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SubjectIndexToolStripMenuItem.Click
        Keyword_Subject_Index.ShowDialog()
    End Sub
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs)
        MsgBox(Extractor.StringRemoveTags("<b> Caggfsgdfgd <i></b>"))
    End Sub
